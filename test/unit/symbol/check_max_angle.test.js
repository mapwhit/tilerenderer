...
test('line with no sharp angles', t => {
  ...
  t.assert.notOk(checkMaxAngle(line, anchor, 25, 20, 0));
});
...
test('one sharp corner', t => {
  ...
  t.assert.notOk(checkMaxAngle(line, anchor, 10, 5, Math.PI / 2 - 0.01));
});
...
test('many small corners close together', t => {
  ...
  t.assert.notOk(checkMaxAngle(line, anchor, 10, 5, Math.PI / 2), 'not allowed if angle within window is big');
...
test('not enough space before the end of the line', t => {
  ...
  t.assert.notOk(checkMaxAngle(line, anchor, 11, 5, Math.PI));
