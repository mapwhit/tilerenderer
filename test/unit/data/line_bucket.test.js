...
function createLineBucket({ id, layout, paint, globalState, overscaling = 1 }) {
  ...
  return new LineBucket({ layers: [layer], globalState, overscaling });
}
...
test('LineBucket', async t => {
  ...
  await t.test('isEmpty method', t => {
    ...
    bucket.addFeature(line, [[new Point(0, 0), new Point(10, 10)]]);
  ...
  await t.test('addLine with different join types', t => {
    ...
    for (const join of joinTypes) {
      for (const cap of capTypes) {
        ...
        const points = [new Point(0, 0), new Point(10, 10), new Point(0, 20), new Point(10, 30)];
  ...
  await t.test('addLine with miter exceeding miterLimit', t => {
    ...
    const points = [
      new Point(0, 0),
      new Point(10, 10),
      new Point(0, 10.1) // Creates a very sharp angle
    ];
  ...
  await t.test('addLine with lineDistances', t => {
    ...
    const points = [new Point(0, 0), new Point(10, 10), new Point(20, 20), new Point(30, 30)];
  ...
  await t.test('addFeatures method', t => {
    ...
    const mockFeatures = [
      {
        geometry: [[new Point(0, 0), new Point(10, 10)]],
        index: 0
      },
      {
        geometry: [[new Point(20, 20), new Point(30, 30)]],
        index: 1
      }
    ];
  ...
  await t.test('addLine with maxed-out distance', t => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      points.push(new Point(i * 100, i * 100));
    }
  ...
  await t.test('addLine with overscaling impact on sharpCornerOffset', t => {
    ...
    const points = [
      new Point(0, 0),
      new Point(100, 100),
      new Point(0, 101) // Creates a very sharp turn
    ];
