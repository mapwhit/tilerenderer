...
test('classifyRings', async t => {
  let feature;
  t.before(() => {
    // Load fill features from fixture tile.
    const sourceLayer = loadVectorTile().layers.water;
    feature = sourceLayer.feature(0);
  });

  await t.test('classifyRings', t => {
    let geometry;
    let classified;
  ...
  await t.test('classifyRings + maxRings', async t => {
